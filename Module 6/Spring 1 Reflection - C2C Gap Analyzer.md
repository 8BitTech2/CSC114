...
> Instructor note: This is the same exemplar reflection, with annotations explaining *why* each part meets the Proficient bar on the rubric rather than Developing. Use this version when walking students through what separates a vague reflection from a strong one.
...
> - Reports results on **all three** test cases, including the two it fails, instead of cherry-picking the passing one. A Developing response often mentions testing only in the abstract, or only reports the case that worked — which hides exactly the information a grader (and the student) needs most.
