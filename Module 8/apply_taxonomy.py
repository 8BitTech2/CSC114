...
# Maps column headers in the expert-coded xlsx to this pipeline's category ids
# (the xlsx has minor naming differences: a typo and one longer name).
EXPERT_COLMAP = {
    "virtualion": "virtualization",
    "networking": "networking",
    "cloud_platforms": "cloud_platforms",
    "storage": "storage",
    "security_compliance": "security_compliance",
    "scripting_automation": "scripting_automation",
    "monitoring_troubleshooting": "monitoring_troubleshooting",
    "hardware_physical_infrastructure": "hardware_physical_infra",
    "itil_soft_skills": "itil_soft_skills",
}
...
def load_expert_labels(cat_ids):
    """Load the instructor's hand-coded course-to-category matrix
    (courses_aligned_with_pillars.xlsx). This replaces keyword weak-labeling
    for the curriculum side entirely -- comparing it against the automated
    classifier's weak-labels gave a Cohen's kappa of 0.073 (slight agreement,
    84 of 144 cells disagreeing, almost all in the direction of the classifier
    under-counting relative to expert judgment). The expert coding is the more
    trustworthy signal, since it reflects what's actually taught rather than
    what a few catalog sentences happen to mention."""
    wb = openpyxl.load_workbook(BASE / "courses_aligned_with_pillars.xlsx", data_only=True)
    ws = wb["Sheet1"]
    ...
    for row in rows[1:]:
        ...
        labels = [0] * len(cat_ids)
        for xlsx_col, cat in EXPERT_COLMAP.items():
            if xlsx_col in row_dict and row_dict[xlsx_col] == 1:
                labels[cat_ids.index(cat)] = 1
        by_course[code] = labels
    return by_course


def weak_label(text, categories):
    """Assign 0/1 labels per taxonomy category via keyword presence.
    Still used for the O*NET job-task side, where no expert coding exists."""
    text_l = text.lower()
    labels = []
...
def build_dataset():
    ...
    expert_labels = load_expert_labels(cat_ids)

    # curriculum snippets -- labels now come from expert coding, not keyword matching
    curr_texts, curr_labels, curr_meta = [], [], []
    for course in curriculum:
        cert_text = " ".join(course.get("industry_certifications", []))
        txt = f"{course['title']}. {course['description']} {cert_text}"
        curr_texts.append(txt)
        curr_labels.append(expert_labels[course["code"]])
        curr_meta.append({"code": course["code"], "title": course["title"]})

    # job-requirement snippets (one per task line, tagged with occupation)
    # still weak-labeled by keyword, since no expert coding exists for O*NET text
...
def run_gap_analysis():
    ...
    # Classifier is trained on expert-labeled curriculum text + weak-labeled
    # job-task text. Training on expert labels (instead of noisy keyword
    # weak-labels) should make the classifier itself more reliable when
    # applied to the O*NET side, even though curriculum scoring below no
    # longer depends on the classifier's predictions at all.
    vectorizer, clf = train_classifier(
        data["curr_texts"] + data["job_texts"],
        np.vstack([data["curr_labels"], data["job_labels"]]),
    )

    # Curriculum coverage now comes directly from the expert-coded matrix --
    # no classifier inference on this side. This is the key change: previously
    # this used classifier-predicted probabilities, which a comparison against
    # the expert coding showed badly under-counted real coverage (Cohen's
    # kappa 0.073 between expert and classifier, "slight" agreement).
    curr_coverage_raw = data["curr_labels"].sum(axis=0)  # count of courses expert-marked per category
    ...
    results["curriculum_coverage_source"] = "expert-coded (courses_aligned_with_pillars.xlsx), not classifier-predicted"
